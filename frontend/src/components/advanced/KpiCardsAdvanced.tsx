...
export function KpiCardsAdvanced({ snapshots, activeListingCount }: Props) {
  ...
  const cards = [
    { label: cs.advanced.kpi.aktivniInzeraty, value: totalActive.toLocaleString("cs-CZ"), tone: "brand" as const },
    { label: cs.advanced.kpi.medianDnuNaTrhu, value: medianDom !== null ? `${medianDom} dní` : "—", tone: "default" as const },
    { label: cs.advanced.kpi.podilSnizeniCeny, value: avgDropShare !== null ? formatPercentPlain(avgDropShare * 100, 0) : "—", tone: "accent" as const },
    { label: cs.advanced.kpi.medianZmenyCeny, value: formatPercent(medianChangePct), tone: "default" as const },
  ];
